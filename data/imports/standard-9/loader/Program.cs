...
var source="data/imports/standard-9/standard-9.extracted.json";
...
var component=await Scalar("SELECT \"componentId\" FROM components WHERE \"componentNumber\"=$1","3");
...
if(await Scalar("SELECT count(*) FROM standards WHERE \"standardNumber\"=$1","9")!=0)throw new Exception("Standard 9 already exists; no changes made");
...
var sid=await Insert("standards","standardId",new(){["standardNumber"]="9",["standardTitle"]=S(data,"standardTitle"),["standardSummary"]=S(data,"standardSummary"),["componentId"]=component,["functionId"]=function});
...
if(expected.Count!=51)throw new Exception("Unexpected record count");
...
if(Convert.ToInt32(await check.ExecuteScalarAsync())!=32)throw new Exception("Post-commit verification failed");
var receipt=new{standardId=sid,componentId=component,functionId=function,criteria=8,compliances=10,evidence=32,verifiedRecords=expected.Count,importedAt=DateTimeOffset.UtcNow};
File.WriteAllText("data/imports/standard-9/import-receipt.json",JsonSerializer.Serialize(receipt,new JsonSerializerOptions{WriteIndented=true}));
