...
var source="data/imports/standard-6/standard-6.extracted.json";
...
if(await Scalar("SELECT count(*) FROM standards WHERE \"standardNumber\"=$1","6")!=0)throw new Exception("Standard 6 already exists; no changes made");
...
var sid=await Insert("standards","standardId",new(){["standardNumber"]="6",["standardTitle"]=S(data,"standardTitle"),["standardSummary"]=S(data,"standardSummary"),["componentId"]=component,["functionId"]=function});
...
if(expected.Count!=63)throw new Exception("Unexpected record count");
...
if(Convert.ToInt32(await check.ExecuteScalarAsync())!=36)throw new Exception("Post-commit verification failed");
var receipt=new{standardId=sid,componentId=component,functionId=function,criteria=13,compliances=13,evidence=36,verifiedRecords=expected.Count,importedAt=DateTimeOffset.UtcNow};
File.WriteAllText("data/imports/standard-6/import-receipt.json",JsonSerializer.Serialize(receipt,new JsonSerializerOptions{WriteIndented=true}));
