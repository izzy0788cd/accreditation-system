...
var source="data/imports/standard-11/standard-11.extracted.json";
...
if(await Scalar("SELECT count(*) FROM standards WHERE \"standardNumber\"=$1","11")!=0)throw new Exception("Standard 11 already exists; no changes made");
...
var sid=await Insert("standards","standardId",new(){["standardNumber"]="11",["standardTitle"]=S(data,"standardTitle"),["standardSummary"]=S(data,"standardSummary"),["componentId"]=component,["functionId"]=function});
...
if(expected.Count!=31)throw new Exception("Unexpected record count");
...
if(Convert.ToInt32(await check.ExecuteScalarAsync())!=14)throw new Exception("Post-commit verification failed");
var receipt=new{standardId=sid,componentId=component,functionId=function,criteria=2,compliances=14,evidence=14,verifiedRecords=expected.Count,importedAt=DateTimeOffset.UtcNow};
File.WriteAllText("data/imports/standard-11/import-receipt.json",JsonSerializer.Serialize(receipt,new JsonSerializerOptions{WriteIndented=true}));
