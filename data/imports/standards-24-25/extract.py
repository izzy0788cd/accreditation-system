...
OUT=Path('data/imports/standards-24-25');NS={'w':'http://schemas.openxmlformats.org/wordprocessingml/2006/main'};W='{'+NS['w']+'}'
def roman(n):
 s=''
...
all_data=[]
for L in ['24','25']:
 raw=json.loads((OUT/f'{L}.raw.json').read_text(encoding='utf-8'));p=Path(raw['source'])
 ...
 number=L;title=tables[0][0][0].split(':',1)[1].strip().capitalize()
 ...
 for rn,row in enumerate(tables[1][2 if L=='24' else 6:],2 if L=='24' else 6):
  if len(row)==2 and re.fullmatch(r'\d+\.\d+',row[0]):
   ...
   criterion['guidance']+='\n'+row[0]
  elif len(row)>=7 and re.fullmatch(r'\d+\.\d+\.\d+',row[0]):
   fixes={}
   if (L,rn) in fixes:
 ...
 if L!='24' and tables[1][1][0].strip():data['standardSummary']+='\n\nService scope\n'+tables[1][1][0].strip()
 if L!='24' and tables[1][3][0].strip():data['sourceReferences']+='\n'+tables[1][3][0].strip()
 titles={'24':'Medication management','25':'Surgical safety'}
 ...
 if L=='25':data['notes'].append('Source reference names Standard 17 although this document is Standard 25; retained verbatim for review.')
