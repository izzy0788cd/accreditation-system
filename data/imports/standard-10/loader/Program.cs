...
var source="data/imports/standard-10/standard-10.extracted.json";
...
if(await Scalar("SELECT count(*) FROM standards WHERE \"standardNumber\"=$1","10")!=0)throw new Exception("Standard 10 already exists; no changes made");
...
var sid=await Insert("standards","standardId",new(){["standardNumber"]="10",["standardTitle"]=S(data,"standardTitle"),["standardSummary"]=S(data,"standardSummary"),["componentId"]=component,["functionId"]=function});
...
if(expected.Count!=19)throw new Exception("Unexpected record count");
...
var receipt=new{standardId=sid,componentId=component,functionId=function,criteria=4,compliances=5,evidence=9,verifiedRecords=expected.Count,importedAt=DateTimeOffset.UtcNow};
File.WriteAllText("data/imports/standard-10/import-receipt.json",JsonSerializer.Serialize(receipt,new JsonSerializerOptions{WriteIndented=true}));
