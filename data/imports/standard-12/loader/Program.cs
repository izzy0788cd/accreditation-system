...
var source="data/imports/standard-12/standard-12.extracted.json";
...
if(await Scalar("SELECT count(*) FROM standards WHERE \"standardNumber\"=$1","12")!=0)throw new Exception("Standard 12 already exists; no changes made");
...
var sid=await Insert("standards","standardId",new(){["standardNumber"]="12",["standardTitle"]=S(data,"standardTitle"),["standardSummary"]=S(data,"standardSummary"),["componentId"]=component,["functionId"]=function});
...
if(expected.Count!=20)throw new Exception("Unexpected record count");
...
if(Convert.ToInt32(await check.ExecuteScalarAsync())!=9)throw new Exception("Post-commit verification failed");
var receipt=new{standardId=sid,componentId=component,functionId=function,criteria=3,compliances=7,evidence=9,verifiedRecords=expected.Count,importedAt=DateTimeOffset.UtcNow};
File.WriteAllText("data/imports/standard-12/import-receipt.json",JsonSerializer.Serialize(receipt,new JsonSerializerOptions{WriteIndented=true}));
