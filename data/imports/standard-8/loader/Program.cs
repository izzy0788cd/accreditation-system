...
var source="data/imports/standard-8/standard-8.extracted.json";
...
if(await Scalar("SELECT count(*) FROM standards WHERE \"standardNumber\"=$1","8")!=0)throw new Exception("Standard 8 already exists; no changes made");
...
var sid=await Insert("standards","standardId",new(){["standardNumber"]="8",["standardTitle"]=S(data,"standardTitle"),["standardSummary"]=S(data,"standardSummary"),["componentId"]=component,["functionId"]=function});
...
if(expected.Count!=94)throw new Exception("Unexpected record count");
...
if(Convert.ToInt32(await check.ExecuteScalarAsync())!=61)throw new Exception("Post-commit verification failed");
var receipt=new{standardId=sid,componentId=component,functionId=function,criteria=7,compliances=25,evidence=61,verifiedRecords=expected.Count,importedAt=DateTimeOffset.UtcNow};
File.WriteAllText("data/imports/standard-8/import-receipt.json",JsonSerializer.Serialize(receipt,new JsonSerializerOptions{WriteIndented=true}));
