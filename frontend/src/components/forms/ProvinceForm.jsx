...
function ProvinceForm({ initialData, onSubmit, onCancel }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <div><p className="text-xs font-bold uppercase tracking-[0.16em] text-teal-700">Location directory</p><h2 className="mt-1 text-xl font-bold text-[#143c42]">{initialData ? "Edit province" : "Add a province"}</h2></div>
      <div>
        <label className="mb-1.5 block text-sm font-semibold text-[#284e53]">Province name</label>
        <input
          type="text"
          name="provinceName"
      ...
      <div>
        <label className="mb-1.5 block text-sm font-semibold text-[#284e53]">Region</label>
        <select
          name="regionId"
