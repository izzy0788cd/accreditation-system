...
function FacilityForm({ initialData, onSubmit, onCancel }) {
  ...
  const selectClass = "w-full rounded-lg border border-[#b9d6d1] bg-white px-3 py-2.5 text-sm outline-none transition focus:border-[#087c77] focus:ring-2 focus:ring-teal-100 disabled:bg-slate-50";
  ...
  return <form onSubmit={submit} className="space-y-5">
    <div><p className="text-xs font-bold uppercase tracking-[0.16em] text-teal-700">Facility directory</p><h2 className="mt-1 text-xl font-bold text-[#143c42]">{initialData ? "Edit facility" : "Add a facility"}</h2></div>
    ...
    <div className="flex gap-2"><button disabled={!prerequisitesReady} type="submit" className="rounded-lg bg-[#087c77] px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-[#05635f] disabled:cursor-not-allowed disabled:opacity-50">{initialData ? "Save changes" : "Add facility"}</button><button type="button" onClick={onCancel} className="rounded-lg px-4 py-2.5 text-sm font-semibold text-[#527076] transition hover:bg-slate-100">Cancel</button></div>
  </form>;
}
function Field({ label, children }) { return <label className="block"><span className="mb-1.5 block text-sm font-semibold text-[#284e53]">{label}</span>{children}</label>; }
function Select({ label, name, value, onChange, options, valueKey, labelKey, disabled, required }) { return <label className="block"><span className="mb-1.5 block text-sm font-semibold text-[#284e53]">{label}</span><select name={name} value={value} onChange={onChange} required={required} disabled={disabled} className="w-full rounded-lg border border-[#b9d6d1] bg-white px-3 py-2.5 text-sm outline-none transition focus:border-[#087c77] focus:ring-2 focus:ring-teal-100 disabled:bg-slate-50"><option value="" disabled>Select {label.toLowerCase()}</option>{options.map((option) => <option key={option[valueKey]} value={option[valueKey]}>{option[labelKey]}</option>)}</select></label>; }
