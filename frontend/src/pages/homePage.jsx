...
import { getAll, getMySurveyProgress, getMySurveys, getSurveyProgress } from "../api/api";
...
function SurveyWorkflowCard({ roleName }) {
  ...
  return <article className="flex h-full min-h-[20rem] flex-col rounded-xl border border-[#b8d9d3] bg-[#f3faf8] p-5 shadow-[0_8px_24px_rgba(20,60,66,0.06)] sm:p-6"><p className="text-xs font-bold uppercase tracking-[.16em] text-[#16803a]">{admin ? "Survey administration" : "Trainee survey workflow"}</p><h2 className="mt-1 text-lg font-bold text-[#092a5a]">{admin ? "Keep the survey team moving" : "A clear path through each survey"}</h2><ol className="mt-5 space-y-4">{steps.map(([number, title, detail]) => <li key={number} className="flex gap-3"><span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[#16803a] text-xs font-bold text-white">{number}</span><div><p className="text-sm font-bold text-[#092a5a]">{title}</p><p className="mt-0.5 text-sm leading-5 text-[#4b5f7a]">{detail}</p></div></li>)}</ol><Link to="/surveys" className="mt-auto inline-block pt-6 text-sm font-semibold text-[#16803a] hover:underline">Open survey workspace →</Link></article>;
}
...
function HomePage() {
  ...
  const isIndividualSurveyor = auth?.roleName === "Surveyor";
  const isFieldTeamRole = ["Surveyor", "Team Lead"].includes(auth?.roleName);

  useEffect(() => {
    const loadSummary = async () => {
      try {
        const accessibleCards = isFieldTeamRole ? [] : canAccessSurveys ? summaryCards.filter((card) => card.resource !== "surveys") : summaryCards.filter((card) => card.resource !== "surveys");
        const [responses, surveyResponse] = await Promise.all([
          Promise.all(accessibleCards.map((card) => getAll(card.resource))),
          canAccessSurveys ? (isIndividualSurveyor ? getMySurveys() : getAll("surveys")) : Promise.resolve({ data: [] }),
        ]);
        ...
        if (canAccessSurveys) loadedCounts.surveys = surveyData.length;
        setCounts(loadedCounts);
        setSurveys(surveyData);
        if (canAccessSurveys) {
          const progressResponses = await Promise.allSettled(surveyData.map((survey) => (isIndividualSurveyor ? getMySurveyProgress(survey.surveyId) : getSurveyProgress(survey.surveyId))));
    ...
    loadSummary();
  }, [canAccessSurveys, isIndividualSurveyor, isFieldTeamRole]);
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:py-10">
      ...
      {canAccessSurveys && <section className="mt-6 grid gap-4 xl:grid-cols-[1fr_1fr_.72fr]" aria-label="Survey activity dashboard">
        <Reveal className="h-full"><SurveyWorkflowCard roleName={auth?.roleName} /></Reveal>
        <Reveal delay={80} className="h-full"><article className="flex h-full min-h-[20rem] flex-col rounded-xl border border-[#d8e4f0] bg-white p-5 shadow-[0_8px_24px_rgba(20,60,66,0.06)] sm:p-6"><p className="text-xs font-bold uppercase tracking-[0.16em] text-[#16803a]">Active assessment work</p><h2 className="mt-1 text-lg font-bold text-[#092a5a]">Survey scoring progress</h2><p className="mt-1 text-sm text-[#68778c]">A quick view of the survey workspaces currently underway.</p><div className="mt-4">{loading ? <div className="h-36 animate-pulse rounded-lg bg-slate-100" /> : <SurveyProgressChart surveys={surveys} progressBySurvey={surveyProgress} />}</div></article></Reveal>
        <Reveal delay={160} className="h-full"><article className="flex h-full min-h-[20rem] flex-col rounded-xl border border-[#d8e4f0] bg-[#092a5a] p-5 text-white shadow-[0_8px_24px_rgba(20,60,66,0.12)] sm:p-6"><p className="text-xs font-bold uppercase tracking-[0.16em] text-[#bbf7d0]">Survey status</p><h2 className="mt-1 text-lg font-bold">Portfolio snapshot</h2><dl className="mt-6 space-y-4"><div className="flex items-end justify-between border-b border-white/15 pb-3"><dt className="text-sm text-slate-200">Active surveys</dt><dd className="text-3xl font-bold">{loading ? "…" : dashboard.active}</dd></div><div className="flex items-end justify-between border-b border-white/15 pb-3"><dt className="text-sm text-slate-200">Cancelled surveys</dt><dd className="text-3xl font-bold text-[#f4d58d]">{loading ? "…" : dashboard.cancelled}</dd></div></dl><Link to="/surveys" className="mt-auto inline-block pt-5 text-sm font-semibold text-[#bbf7d0] hover:text-white hover:underline">Open the survey workspace →</Link></article></Reveal>
      </section>}
      ...
      {!isFieldTeamRole && <><section className="mt-10" aria-labelledby="workspace-title">
