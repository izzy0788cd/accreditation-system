...
import SurveyorReportPanel from "../../components/SurveyorReportPanel";
import { MAX_SCORE_VALUE } from "../../utils/scoring";
import { aggregateDashboardStandards } from "../../utils/standardFamilies";
...
function SurveyTeamLeadDashboardPage() {
  ...
  const dashboard = useMemo(() => {
    ...
    const dashboardStandardRows = aggregateDashboardStandards(standardRows, MAX_SCORE_VALUE).sort((first, second) => numberSort(first.standardNumber, second.standardNumber));
    const componentRows = components.map((component) => {
      const componentStandards = dashboardStandardRows.filter((standard) => standard.componentNumber === component.componentNumber);
    ...
    return { standardRows: dashboardStandardRows, componentRows, team, risks: rows.filter((row) => row.riskRatingId).length, unscored: rows.filter((row) => !row.scoreId).length, uncheckedEvidence: checks.filter((check) => !check.isChecked).length, assessmentById };
  ...
  return <main className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:py-8">
    ...
    <SurveyorReportPanel surveyId={surveyId} teamView />
