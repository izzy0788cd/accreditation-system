...
function StandardPerformanceChart({ standards }) {
  const tone = (standard) => standard.score == null ? "bg-slate-300" : standard.score < 50 ? "bg-red-500" : standard.score < 70 ? "bg-[#d6aa45]" : "bg-[#087c77]";
  ...
  return <section className="mt-6 overflow-hidden rounded-xl border border-[#dce9e7] bg-white shadow-sm"><div className="flex flex-col gap-3 border-b border-[#dce9e7] px-5 py-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.15em] text-teal-700">Performance overview</p><h2 className="mt-1 font-bold text-[#143c42]">Standard performance</h2><p className="mt-1 text-sm text-[#668187]">Numeric scores only; N/A is excluded from each percentage.</p></div><div className="flex flex-wrap gap-x-3 gap-y-1 text-xs font-semibold text-[#668187]"><span><i className="mr-1 inline-block h-2.5 w-2.5 rounded-full bg-[#087c77]" />70–100%</span><span><i className="mr-1 inline-block h-2.5 w-2.5 rounded-full bg-[#d6aa45]" />50–69%</span><span><i className="mr-1 inline-block h-2.5 w-2.5 rounded-full bg-red-500" />Below 50%</span></div></div><div className="max-h-[34rem] space-y-4 overflow-y-auto p-5 sm:p-6">{standards.map((standard) => <div key={standard.standardId} className="group"><div className="mb-1.5 flex items-baseline justify-between gap-4"><div className="min-w-0"><p className="truncate text-sm font-semibold text-[#143c42]">{standard.standardNumber} · {standard.standardTitle}</p><p className="mt-0.5 text-xs text-[#668187]">{standard.scored} / {standard.entries.length} requirements scored · {status(standard)}</p></div><strong className="shrink-0 text-sm text-[#143c42]">{standard.score == null ? "—" : `${standard.score}%`}</strong></div><div className="h-3 overflow-hidden rounded-full bg-[#e4efed]" title={`${standard.standardTitle}: ${standard.score == null ? "not scored or N/A" : `${standard.score}%`}`}><div className={`h-full rounded-full transition-all duration-700 ${tone(standard)}`} style={{ width: `${standard.score || 0}%` }} /></div></div>)}</div></section>;
}
