...
function FacilityDetailPage() {
  ...
  return <section aria-labelledby="facility-detail-title"><Link to="/facilities/directory" className="mb-5 inline-flex text-sm font-semibold text-[#16803a] hover:underline">← Back to facility directory</Link><div className="rounded-2xl border border-[#c9dded] bg-[linear-gradient(125deg,#eaf3fb_0%,#f8fafc_70%)] p-6 sm:p-8"><p className="text-xs font-bold uppercase tracking-[0.16em] text-[#16803a]">Health Facility</p><h2 id="facility-detail-title" className="mt-2 text-3xl font-bold tracking-tight text-[#092a5a]">{facility.facilityName}</h2><p className="mt-2 text-sm text-[#4b5f7a]">{location || facility.districtName}</p><AccreditationStatusBadge status={facility.creditationStatus} className="mt-5 text-sm" /></div><div className="mt-6 grid gap-4 sm:grid-cols-2">{fields.map(([label, value]) => <div key={label} className="rounded-xl border border-[#dfe7f0] bg-white p-5 shadow-[0_8px_24px_rgba(20,60,66,0.06)]"><p className="text-xs font-bold uppercase tracking-wider text-[#68778c]">{label}</p><p className="mt-2 font-semibold text-[#092a5a]">{value}</p></div>)}</div>{facility.comments && <div className="mt-6 rounded-xl border border-[#dfe7f0] bg-white p-5 shadow-[0_8px_24px_rgba(20,60,66,0.06)]"><h3 className="font-bold text-[#092a5a]">Notes</h3><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-[#4b5f7a]">{facility.comments}</p></div>}<div className="mt-6 rounded-xl border border-dashed border-[#c5d5e8] bg-[#f6f9fc] p-5"><h3 className="font-bold text-[#092a5a]">Surveys</h3><p className="mt-1 text-sm text-[#4b5f7a]">Survey creation will start here once the assessment workflow is added.</p></div></section>;
}
