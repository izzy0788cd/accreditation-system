  ...
  return (
    <section aria-labelledby={`${resource}-title`}>
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="mb-1 text-xs font-bold uppercase tracking-[0.16em] text-[#16803a]">Location Directory</p>
          <h2 id={`${resource}-title`} className="text-2xl font-bold tracking-tight text-[#092a5a]">{plural}</h2>
          <p className="mt-1 text-sm text-[#68778c]">Maintain the {plural.toLowerCase()} used to organise health services.</p>
        </div>
        {canManage && <button onClick={() => { setEditingItem(null); setFormKey((key) => key + 1); setShowForm(true); }} className="inline-flex items-center justify-center gap-2 rounded-lg bg-[#16803a] px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-[#0d6531] focus:outline-none focus:ring-2 focus:ring-[#16803a] focus:ring-offset-2">
      ...
      <div className="overflow-hidden rounded-xl border border-[#dfe7f0] bg-white shadow-[0_8px_24px_rgba(20,60,66,0.06)]">
        {loading ? <div className="space-y-3 p-6" aria-label={`Loading ${plural.toLowerCase()}`}>{[1, 2, 3].map((row) => <div key={row} className="h-10 animate-pulse rounded bg-slate-100" />)}</div>
          : sortedItems.length === 0 ? <div className="px-6 py-14 text-center"><div className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-[#edf8f0] text-xl text-[#16803a]">⌁</div><h3 className="font-semibold text-[#092a5a]">No {plural.toLowerCase()} yet</h3><p className="mt-1 text-sm text-[#68778c]">Add the first {singular.toLowerCase()} to start building the location directory.</p></div>
          : <div className="overflow-x-auto"><table className="w-full min-w-[520px] text-left"><thead className="border-b border-[#dbe5ef] bg-[#f6f9fc] text-xs uppercase tracking-wider text-[#4b5f7a]"><tr>{parentLabel && <th className="px-6 py-3.5 font-semibold">{parentLabel}</th>}<th className="px-6 py-3.5 font-semibold">{singular} name</th>{canManage && <th className="px-6 py-3.5 text-right font-semibold">Actions</th>}</tr></thead><tbody className="divide-y divide-[#e7edf4] text-sm">{groupedItems.flatMap((group) => group.map((item, index) => <tr key={item[idField]} className={`transition-colors hover:bg-[#f5f9fd] ${parentField && index === 0 ? "border-t-2 border-[#c5d5e8]" : ""}`}>{parentField && index === 0 && <td rowSpan={group.length} className="border-r border-[#e7edf4] bg-[#f6f9fc] px-6 py-4 align-top font-semibold text-[#16803a]">{item[parentField] || "Unassigned"}</td>}<td className="px-6 py-4 font-semibold text-[#092a5a]">{item[nameField]}</td>{canManage && <td className="px-6 py-4"><div className="flex justify-end gap-2"><button onClick={() => { setEditingItem(item); setShowForm(true); }} className="rounded-md border border-[#c5d5e8] px-3 py-1.5 text-xs font-semibold text-[#16803a] transition hover:bg-[#edf8f0]">Edit</button><button onClick={() => setDeleteTarget(item)} className="rounded-md border border-red-200 px-3 py-1.5 text-xs font-semibold text-red-700 transition hover:bg-red-50">Delete</button></div></td>}</tr>))}</tbody></table></div>}
      </div>
